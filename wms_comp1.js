/* ============================================================
   WMS ANALÍTICO — wms_comp1.js  [v2.1]
   Aba "CD 1 × 3 × 7" — comparação de saldos ARM 1 apenas
   cd normalizado: '001' → '1', '003' → '3', '007' → '7'
   ============================================================ */
...
function renderComp1() {
  ...
  if (filtered.length === 0) {
    tbody.innerHTML = '<tr><td colspan="7" class="empty-state"><p>Nenhum resultado encontrado.</p></td></tr>';
  } else {
    tbody.innerHTML = page.map(r => {
      const hasCrit = [r.v_cd1, r.v_cd3, r.v_cd7].some(v => v !== null && v < CRITICAL);
      const cls = hasCrit ? 'row-critical' : '';
      return '<tr class="' + cls + '"><td><code style="font-family:var(--mono);font-size:11px;color:var(--accent)">' + (r.cd_material||'—') + '</code></td><td title="' + r.desc_material + '">' + (r.desc_material||'—') + '</td><td class="td-num">' + fmtNum(r.v_cd1) + '</td><td class="td-num">' + fmtNum(r.v_cd3) + '</td><td class="td-num">' + fmtNum(r.v_cd7) + '</td><td class="td-num" style="font-family:var(--mono);font-size:12px">' + fmt(r.total, false) + '</td><td>' + buildComp1TransferHint(r) + '</td></tr>';
    }).join('');
  }
